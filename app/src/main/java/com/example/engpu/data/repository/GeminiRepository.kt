...
class GeminiRepository {
    ...
    suspend fun getFeedbackForInterview(
        ...
        val prompt = """
            당신은 10년 이상의 경력을 가진 전문 기술 면접관입니다.
            실제 채용 면접과 동일한 수준의 엄격하고 진지한 평가를 수행해주세요.
            
            지원자가 제공한 답변을 다음 기준으로 평가하십시오:
            - 질문의 핵심을 정확히 이해했는가
            - 답변의 논리성과 구조가 명확한가
            - 실무 경험과 기술적 깊이가 드러나는가
            - 커뮤니케이션 능력이 우수한가
            
            면접 질문: $question
            지원자 답변: $answer
            
            다음 형식으로 전문적인 피드백을 작성해주세요:
            
            1. 답변의 강점 (구체적이고 객관적으로)
            - 어떤 부분이 우수했는지 명확히 기술
            
            2. 개선이 필요한 부분 (명확한 근거와 함께)
            - 부족한 점과 그 이유를 구체적으로 설명
            - 실무 관점에서 왜 문제가 되는지 제시
            
            3. 평가 점수: X/10
            - 1-4점: 불합격 수준 (기본 이해 부족)
            - 5-6점: 보통 수준 (추가 학습 필요)
            - 7-8점: 우수 (실무 투입 가능)
            - 9-10점: 매우 우수 (즉시 채용 가능)
            
            4. 실무 관점의 개선 제안
            - 실제 업무 상황에서 필요한 구체적 개선 방향
            - 추가로 학습하면 좋을 기술이나 개념
            
            평가는 엄격하고 객관적으로 진행하되, 건설적인 피드백을 제공해주세요.
        """.trimIndent()
